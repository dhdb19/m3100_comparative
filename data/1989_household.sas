...
FILE FROM THE NHIS 1989 HOUSEHLD.DAT ASCII FILE.
...
FILENAME ASCIIDAT 'C:\NHIS1989\HOUSEHLD.DAT';
...
INPUT
   ...
   NONIVIEW     28 - 29        MONTH    $   30 - 31
   DAY      $   32 - 33        NFAM         34 - 35
...
LABEL
   ...
   MONTH    = "MONTH OF COMPLETION (OR LAST ATTEMPT) OF INITIAL INTERVIEW"
   DAY      = "DAY OF COMPLETION (OR LAST ATTEMPT) OF INITIAL INTERVIEW"
...
FORMAT
   ...
   HHNUM     $HLD007X.   WEEKPROC  HLD008X.    LIVQTR    HLD009X.
   PHONE     HLD010X.    SPLACE    HLD011X.    COMIVIEW  HLD012X.
   NONIVIEW  HLD013X.    MONTH     $HLD014X.   DAY       $HLD015X.
   NFAM      HLD016X.    SIZEPRIM  HLD017X.    SIZESEC1  HLD018X.
   SIZESEC2  HLD019X.    SIZESEC3  HLD020X.    SIZESEC4  HLD021X.
   SIZESEC5  HLD022X.    SIZESEC6  HLD023X.    SIZESEC7  HLD024X.
   SIZESEC8  HLD025X.    SIZESEC9  HLD026X.    NUNREL    HLD027X.
   REFPRESP  HLD028X.    REFPRES1  HLD029X.    REFPRES2  HLD030X.
   REFPRES3  HLD031X.    REFPRES4  HLD032X.    REFPRES5  HLD033X.
   REFPRES6  HLD034X.    REFPRES7  HLD035X.    REFPRES8  HLD036X.
   REFPRES9  HLD037X.    NPERSREC  HLD038X.    NCONDREC  HLD039X.
   NDVREC    HLD040X.    NHOSPREC  HLD041X.    CONDLIST  HLD042X.
   SUBTYPE   HLD043X.    REGION    HLD044X.    MSASIZE   HLD045X.
   PSUTYPE   HLD046X.    MSA       HLD047X.    WTFQ      HLD048X.
   ;
 
PROC CONTENTS DATA=NHIS.HOUSEHLD;

PROC FREQ DATA=NHIS.HOUSEHLD;
TITLE1 'FREQUENCY REPORT FOR 1989 NHIS PUBLIC USE HOUSEHOLD FILE';
TITLE2 '(WEIGHTED)';
TABLES RECTYPE/LIST MISSING;
WEIGHT WTFA_HH;
 
PROC FREQ DATA=NHIS.HOUSEHLD;
TITLE1 'FREQUENCY REPORT FOR 1989 NHIS HOUSEHOLD PUBLIC USE FILE';
TITLE2 '(UNWEIGHTED)';
TABLES RECTYPE/LIST MISSING;
